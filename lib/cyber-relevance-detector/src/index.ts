...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  Malware: ["malware", "virus", "trojan", "worm", "rootkit", "keylogger", "backdoor", "botnet"],
  Phishing: ["phishing", "spearphishing", "whaling", "social engineering"],
  Vulnerability: ["vulnerability", "cve", "patch", "security update", "zero-day", "exploit"],
  "Data Breach": ["breach", "leak", "exposure", "compromised", "stolen data"],
  APT: ["apt", "advanced persistent", "nation-state", "state-sponsored"],
  DDoS: ["ddos", "denial of service"],
  "Web Security": ["xss", "sql injection", "csrf", "web application"],
  "Network Security": ["network intrusion", "firewall", "ids", "ips"],
  "Cloud Security": ["cloud security", "aws", "azure", "misconfiguration"],
  "IoT/ICS": ["iot", "ics", "scada", "industrial control"],
  Fraud: ["fraud", "scam", "identity theft", "credential theft"],
};
...
export class CyberSecurityRelevanceDetector {
  isRelevant(
    ...
    if (metadata?.source) {
      const isSecuritySource = SECURITY_SOURCES.some((src) =>
        metadata.source!.toLowerCase().includes(src)
      );
      if (isSecuritySource) {
        relevanceScore += 40;
    ...
    return {
      ...
      category: category ?? this.guessCategory(lowerText),
  ...
  private guessCategory(text: string): string {
    const categoryPatterns: Record<string, RegExp> = {
      Ransomware: /ransomware|ransom\s+demand|encrypted\s+files/i,
      Malware: /malware|trojan|virus|worm|botnet/i,
      Phishing: /phishing|fake\s+(?:email|site|login)/i,
      Vulnerability: /vulnerability|cve-|security\s+flaw|patch/i,
      "Data Breach": /data\s+(?:breach|leak)|exposed\s+(?:data|records)/i,
      APT: /apt|nation-state|state-sponsored/i,
      Fraud: /fraud|scam|financial\s+crime/i,
      DDoS: /ddos|denial\s+of\s+service/i,
    };
