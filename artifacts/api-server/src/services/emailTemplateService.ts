...
export class EmailTemplateService {
  ...
  prepareData(advisory: AdvisoryWithCustomizations): PreparedData {
    const isThreat = advisory.isThreat ?? false;
    const links = isThreat
      ? normalizeThreatLinks(advisory)
      : normalizeAdvisoryLinks(advisory);
    ...
    return {
      ...
      references: this.cleanArray(links.references),
...
<p style="color:#6B7280;font-size:12px;margin:0 0 8px;font-family:Arial,sans-serif;"><a href="${data.sourceUrl}" style="color:${c.primary};text-decoration:none;">${getPrimaryAdvisoryLinkLabel({ source: data.source, isCertIn: data.isCertIn })}</a></p>
<p style="color:#9CA3AF;font-size:11px;margin:0;font-family:Arial,sans-serif;">Prepared by Security Advisory Team</p></div>
</div></body></html>`;
  }
...
${this.renderFooter(data, c.primary)}
  ...
  private genPlainText(data: PreparedData): string {
    ...
    const heading = data.isThreat ? "THREAT INTELLIGENCE ALERT" : "SECURITY ADVISORY";
  ...
  private renderFooter(data: PreparedData, accentColor: string): string {
    const sourceLabel = data.isThreat
      ? "View Threat Source"
      : getPrimaryAdvisoryLinkLabel({ source: data.source, isCertIn: data.isCertIn });
    return `<div style="background:#1F2937;padding:20px 28px;"><table width="100%" cellpadding="0" cellspacing="0"><tr><td><p style="color:#9CA3AF;margin:0 0 6px;font-size:12px;"><a href="${data.sourceUrl}" style="color:${accentColor};text-decoration:none;">${sourceLabel}</a></p><p style="color:#6B7280;margin:0;font-size:11px;">Prepared by Security Advisory Team • ${data.currentDateTime}</p></td></tr></table></div>`;
  }
